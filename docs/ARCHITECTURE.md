...
| Code validation | GitHub Actions + Bun checks | `.github/workflows/ci.yml` runs frozen installs, high-severity `bun audit`, lint, typecheck, and tests for `web/`, plus a frozen install and high-severity audit for `pipeline/` on PRs and `main` pushes |
