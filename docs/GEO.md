...
- Org pages emit `orgLd` with `sameAs` arrays: the GitHub owner URL is mandatory, and optional additions must come from the reviewed static registry.
...
GEO uses the following shapes. Dataset and `dateModified` are implemented for home, ranking, category, and methodology surfaces; FAQPage is implemented from visible FAQ blocks; repo/org `sameAs` arrays are implemented with mandatory GitHub identities plus deterministic repo homepage or approved static-registry additions.
...
- Implemented in #56: `faqPageLd(items, path, locale)` lives in `web/lib/jsonld.ts`.
- FAQ answers are generated by deterministic helpers and rendered through one visible server component.
- The compare page emits only generic static FAQ for the tool shell; query-state selections remain client-only and are not described as server-rendered evidence.

### 6.3 Repo and organization sameAs

`repoLd` and `orgLd` emit `sameAs` arrays for entity disambiguation. The GitHub identity URL is mandatory for both repo and owner pages. Optional additions are restricted to deterministic repo metadata (`homepage_url`) or reviewed static registry entries; no request-time scraping or arbitrary profile discovery is allowed.
...
```json
{
  ...
  "name": "owner/project",
  "url": "https://gitstarclub.com/owner/project",
  "codeRepository": "https://github.com/owner/project",
  "sameAs": [
...
The non-GitHub URL above is emitted only when it is the repository's existing `homepage_url` field in the deterministic repo entity JSON.
...
- Keep GitHub sameAs mandatory when rendering repo and owner pages.
- Add repository homepage sameAs only from the existing `homepage_url` field in repo entity JSON; do not discover or crawl homepage links at request time.
- Add optional owner/repo registry enrichment only from `APPROVED_OWNER_SAME_AS_REGISTRY` or `APPROVED_REPO_SAME_AS_REGISTRY` in `web/lib/jsonld.ts`; the registries currently ship empty.
- If Wikidata or other profiles are added, record source and ownership in the registry and docs before shipping.
...
| Repo/org sameAs | GitHub repo or owner URLs, repo homepage metadata, approved static-registry entries, and later Wikidata only if separately approved. | Helps entity disambiguation across Google and answer engines. |
...
- Keep GitHub repo and owner URLs as the mandatory baseline `sameAs` value for repo, organization, and person entity pages.
...
| `[geo] Add visible FAQ blocks and FAQPage JSON-LD` | Implemented in #56: 3-5 deterministic FAQ items per page type and matching FAQPage schema from a shared builder. | FAQ is visible in HTML; JSON-LD exactly mirrors visible answers; no hidden schema; tests cover escaping and schema shape. |
