...
| `WORKFLOW_COLD_START` | 预发首次 managed publish 是否允许从零 bootstrap canonical / 空 lookup | 可选（默认关） | 字符串 `"1"` 才开；仅预发 wrangler `env.pre`，且应与 `PREFLIGHT_RELAX_EMPTY_SHARDS=1` 同开。`VERCEL_ENV=production` 即使误设也拒绝 | `web/lib/workflows/cold-start.ts` · `web/lib/runtime-config.ts` · refresh start / preflight / whitelist / metadata / recompute I/O。只在 `views/latest.json` 仍 404 时生效；写入的 meta 使用真实 UTC `generated_at` 与 bootstrap 日 `seam_date`（无 GH Archive  gross 历史），**禁止**假 `node_id`/假 GraphQL。与 #515 空桶占位、#512 502 恢复、SafeText 冲突时 **以冷启动为准**（见 [VERCEL-DATA-OPERATIONS.md](./VERCEL-DATA-OPERATIONS.md)）。首 publish 成功后自动回到 fail closed。回滚 = 去掉或设 `0` |
| `WHITELIST_SEARCH_HOP_BUDGET_MS` | 单 hop Search 墙钟预算 | 可选（默认 `600000` = 10 min） | 整数毫秒，`60000..840000`（必须低于 Queue 15 min wall） | `web/lib/runtime-config.ts` · `web/lib/github.ts` `searchWhitelistHop` |
...
| `VERCEL_AUTOMATION_BYPASS_SECRET` | Preview Vercel Authentication 的 CI bypass（`x-vercel-protection-bypass`） | 仅当 preview-e2e / product-gates 解析到 Vercel Preview 时需要；Ignored Build / 无 Preview 时 skip | Vercel Protection Bypass for Automation token | `web/lib/vercel-protection-bypass.ts` · `web/scripts/resolve-vercel-preview.ts`；不进浏览器、不进生产页面 |
...
| **每周 refresh workflow** | `0 6 * * 0`（周日 06:00） | **Vercel cron + 无 SDK 编排 / 全量刷新**：`/api/workflows/refresh/start` 鉴权后取得 lease 并 `startRefresh`——白名单 → 改名 → 元数据 → 折叠 → rank / entity / heatmap 重算 → 校验 → 发布（切指针）→ 版本 GC。默认 HTTP 自链到 `/api/workflows/refresh/step`；生产排程仍是本表，不是 CF Cron | **否**（发布只切指针；排程独立于 daily / weekly） |
...
**为什么拆 step 而不是单 Function**：单 Function 上限 800s / 4GB / bundle 250MB / 响应体 4.5MB（[Functions Limits](https://vercel.com/docs/functions/limitations)），装不下 DuckDB 全量重算。P1 不再依赖 Vercel Workflow SDK；每步是普通 async + 显式重试，由 HTTP 自链或非生产 CF Queue 推进。生产排程仍是 Vercel cron，不要为了新编排去关 Fluid。

**部署前置**：
- Fluid Compute 可保持开启（P1 **禁止**为了赌新编排而关掉它）。
- env：`CRON_SECRET`、`GITHUB_TOKEN`、`BLOB_READ_WRITE_TOKEN`、`BLOB_BASE_URL` 同时配 Production 与 Preview。`WORKFLOW_RUNTIME` 默认 `http`。
- 部署：`vercel deploy . --yes --scope zkscio --project gitstarclub.com`（preview，从仓库根；Root Directory=web）。

**手动触发 runbook**：

1. `GET <deployment>/api/workflows/refresh/start`，带 `Authorization: Bearer <CRON_SECRET>` → route 先只读校验 `canonical/v2/meta.json` 与全部 32 个 `repos` shard（含 `active` / `tracked_since` / `d`、key/id/bucket），通过后才取得 lease 并 `startRefresh`，随即返回 `run_id`（不阻塞）。preflight 失败时不会 enqueue 或取得 lease；step `preflight` 会在任何 canonical mutation 前再校验全部 128 个必需 shard。CF / HTTP 编排把这次复核拆成 4-bucket 窗口（每窗口 16 个 shard，`ops/workflows/<run_id>/steps/preflight-0.json` … `preflight-28.json`），避免 Workers 一次读齐 128 个对象触发 1102。预发 `PREFLIGHT_RELAX_EMPTY_SHARDS=1` 时，确认 404 / 空桶当作 `{}` 占位并记 `preview-empty-canonical-placeholder` 日志，不整 run 作废；生产保持 fail closed。
...
> 全链路 step：`preflight`（再次校验全部 canonical shard）→ `fold`（月 + 周；每次先写 `fold-decision.json`。有封闭 pending 时再写紧凑 `fold-month-plan.json` / `fold-week-plan.json`，按 1-bucket 窗口拆成 `fold-month-*` / `fold-week-*`，最后仍写 `fold.json`。已折到当前月且无剩余周则 `reason=no_closed_month`，没有 month plan 不是卡死）→ `recomputeRank`（CF 上 month-pack + 8-period month/monthOrg，year 从 month bucket 派生后再 8-period year/yearOrg，再 week-pack + 8-period week/weekOrg，最后 rest 按 repos bucket；fold 后写 `recompute-enqueued.json`，每 hop 先写 `recomputeRank-<phase>-start.json`。#497 的 week 整窗 OOM 已按桶拆开；month/year/rest 同样不再整窗装载，见 [CF-MIGRATION-P1.md](./CF-MIGRATION-P1.md)）→ `buildAliases`（→ `lookup/aliases.json`）→ `validate` 发布闸门 → `publish` 切 `views/latest.json` 指针 / 回滚 → `gc` 版本回收（设计见 [VERCEL-DATA-OPERATIONS.md](./VERCEL-DATA-OPERATIONS.md) §7）。CF 预发 Blob 文件名是运行时 step：fold 之后应出现 `recompute-enqueued.json`、`recomputeRank-month-pack.json` / `recomputeRank-month-*.json` 直到终端 `recomputeRank-month.json`，以及 year-pack / week-pack / `recomputeRank-rest-*.json` 和终端 `recomputeRank.json`（不是清单别名 `recompute.json`）。`WORKFLOW_RUNTIME=cf-queue` 时由 Queue consumer 读 step JSON（或 `x-gitstarclub-queue-successor`）再 `JOBS.send` 下一步；只写出 `fold-decision`+`fold.json` 后队列沉默、fetch 源 `Worker exceeded memory limit`、无 `error.json`、租约过期是 #486 后仍未收口的卡死（根因是 month hop 仍装对象窗口，不是 fold-decision 没入队；见 [CF-MIGRATION-P1.md](./CF-MIGRATION-P1.md)）。`/` `/rankings` 500 而 `/preview/health` 200 是同一 isolate OOM（OpenNext vs shell），不是 published 被写坏。预发 Bearer 全量 refresh 的通过/失败表见 [CF-MIGRATION-P1.md](./CF-MIGRATION-P1.md#preview-bearer-full-refresh-acceptance-matrix)。OOM 或 Queue 沉默算失败，不是“还在跑”。
