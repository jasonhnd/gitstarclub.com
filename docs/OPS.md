...
| `WHITELIST_SEARCH_SHARDS` | whitelist GitHub Search 是否按 hop 分片 | 可选（默认关 = 单 hop） | 字符串 `"1"` 才开；预发 wrangler `env.pre` 为 `1` | `web/lib/runtime-config.ts` · `web/lib/workflows/steps/whitelist.ts` · `web/lib/github.ts`；每 hop 写 `ops/workflows/<run_id>/whitelist-search.json`，完成才写 snapshot。回滚 = 去掉或设 `0` 恢复单 hop |
...
> 全链路 step：`preflight`（再次校验全部 canonical shard）→ `fold`（月 + 周；每次先写 `fold-decision.json`。有封闭 pending 时再写紧凑 `fold-month-plan.json` / `fold-week-plan.json`，按 1-bucket 窗口拆成 `fold-month-*` / `fold-week-*`，最后仍写 `fold.json`。已折到当前月且无剩余周则 `reason=no_closed_month`，没有 month plan 不是卡死）→ `recomputeRank`（CF 上拆成 packed month / monthOrg / year / yearOrg，再 week-pack + 8-period week / weekOrg 流式窗口，最后 rest；fold 后写 `recompute-enqueued.json`，每 hop 先写 `recomputeRank-<phase>-start.json`。#497 仍会在 `recomputeRank-week-start` 整窗装载 week-win 后 OOM；见 [CF-MIGRATION-P1.md](./CF-MIGRATION-P1.md)）→ `buildAliases`（→ `lookup/aliases.json`）→ `validate` 发布闸门 → `publish` 切 `views/latest.json` 指针 / 回滚 → `gc` 版本回收（设计见 [VERCEL-DATA-OPERATIONS.md](./VERCEL-DATA-OPERATIONS.md) §7）。CF 预发 Blob 文件名是运行时 step：fold 之后应出现 `recompute-enqueued.json` 与 `recomputeRank-month.json` … `recomputeRank-rest.json` 和终端 `recomputeRank.json`（不是清单别名 `recompute.json`）。`WORKFLOW_RUNTIME=cf-queue` 时由 Queue consumer 读 step JSON（或 `x-gitstarclub-queue-successor`）再 `JOBS.send` 下一步；只写出 `fold-decision`+`fold.json` 后队列沉默、fetch 源 `Worker exceeded memory limit`、无 `error.json`、租约过期是 #486 后仍未收口的卡死（根因是 month hop 仍装对象窗口，不是 fold-decision 没入队；见 [CF-MIGRATION-P1.md](./CF-MIGRATION-P1.md)）。`/` `/rankings` 500 而 `/preview/health` 200 是同一 isolate OOM（OpenNext vs shell），不是 published 被写坏。预发 Bearer 全量 refresh 的通过/失败表见 [CF-MIGRATION-P1.md](./CF-MIGRATION-P1.md#preview-bearer-full-refresh-acceptance-matrix)。OOM 或 Queue 沉默算失败，不是“还在跑”。
