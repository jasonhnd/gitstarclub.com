...
source_of_truth_for:
  ...
  - UI/UX roadmap route reconciliation
---

# UI/UX Route Inventory

This is the authoritative route inventory for Phase 0 UI/UX work. It was verified
against the current Next.js App Router tree on branch `loop/issue-188`.
...
and `fr`. The default English prefix `/en/*` is not canonical and middleware
...
| Repository detail | `/{owner}/{name}` | `/react/react` | `web/app/(en)/[owner]/[name]/page.tsx`, `web/app/(localized)/[locale]/[owner]/[name]/page.tsx`, `web/app/_localized/repo.tsx`, `web/lib/repo-route.ts`, `web/lib/repo-page.ts` | ISR (`dynamicParams=true`, `revalidate=86400`); repo params are generated on demand | `/{locale}/{owner}/{name}` | Yes for every repo in `lookup/repos.json` | `getRepoIdByFullNameDaily()`, `getAliasMapDaily()`, `getReposLookupDaily()`, `getRepoEntityDaily(id)`, `getCategoryAssignments()`, `getCategoryRegistry()`, `getMeta()`; Blob views `lookup/repos.json`, `lookup/aliases.json`, `entity/repo/{id}.json`, `categories/assignments.json`, `categories/registry.json`, `meta.json` |
...
| Search index JSON | `/search-index` | `/search-index` | `web/app/search-index/route.ts`, `web/lib/data/search.ts` | Dynamic route handler with CDN cache (`s-maxage=3600`, stale while revalidate 86400) | None; middleware ignores it | No | `getSearchIndex()`; Blob view `search/index.json`; descriptions are truncated before response |
| Repo curve JSON | `/repo-curve?id={id}` | `/repo-curve?id=10270250` | `web/app/repo-curve/route.ts`, `web/lib/data/compare.ts` | Dynamic route handler with CDN cache (`s-maxage=3600`, stale while revalidate 86400) | None; middleware ignores it | No | `getRepoCurve(id)` projects `entity/repo/{id}.json` into the compare curve payload |
...
| Default OG image | `/opengraph-image` | `/opengraph-image` | `web/app/opengraph-image.tsx`, `web/lib/og-theme.ts` | ISR image route (`revalidate=86400`) | None; middleware ignores OG image paths | No | Static OG theme constants |
| Repo OG image | `/{owner}/{name}/opengraph-image` | `/react/react/opengraph-image` | `web/app/(en)/[owner]/[name]/opengraph-image.tsx` | ISR image route (`revalidate=86400`) | No localized OG file currently exists for repo pages | No | `getRepoIdByFullNameDaily()`, `getRepoEntityDaily(id)`; Blob views `lookup/repos.json`, `entity/repo/{id}.json` |
...
| Robots | `/robots.txt` | `/robots.txt` | `web/app/robots.ts` | Static metadata route, environment-sensitive | None; middleware ignores it | No | `SITE_INDEXABLE` controls preview disallow vs public allow rules; when public, advertises `/sitemap.xml` and disallows `/api/` |
...
Authenticated operational routes are intentionally excluded from the public UI
inventory: `/api/cron/daily`, `/api/cron/weekly`, and
`/api/workflows/refresh/start` are route handlers for cron/workflow operations,
not public navigation surfaces.
...
|---|---|---|
| Repo detail is `/repo/{owner}/{name}` or another `/repo/...` namespace | MISMATCH | Repo detail is `/{owner}/{name}`. Files are `web/app/(en)/[owner]/[name]/page.tsx` and `web/app/(localized)/[locale]/[owner]/[name]/page.tsx`. |
...
| Route files live directly under pre-split paths such as `web/app/rankings/...` | MISMATCH | Public page files are split into `web/app/(en)/...` and `web/app/(localized)/[locale]/...`, with shared implementations in `web/app/_localized/*.tsx`. Root handlers remain under `web/app/*/route.ts` or metadata files. |
