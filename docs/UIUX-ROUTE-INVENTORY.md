...
| Robots | `/robots.txt` | `/robots.txt` | `web/app/robots.ts` | Static metadata route, environment-sensitive | None; `web/proxy.ts` ignores it | No | `SITE_INDEXABLE` controls preview disallow vs public allow rules; when public, advertises `/sitemap.xml` and disallows `/api/` |
