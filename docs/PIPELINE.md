...
**01 whitelist** — GitHub Search `stars:>=MIN_TRACKED_STARS`（默认 `web/lib/constants.ts` = 10,000；运行时 `MIN_TRACKED_STARS` 可覆盖，预发 wrangler `env.pre` = 1,000）只做成员发现。先用开放上界查询当前最高 star，再以该动态上界按 star 区间**自适应分桶**绕过 Search 1000 结果上限（区间 >1000 则二分）；没有 600k 固定上限。预发 `WHITELIST_SEARCH_SHARDS=1` 时，Search 队列按 hop 消化（默认 10 min / hop），进度写 `ops/workflows/<run_id>/whitelist-search.json`，避免单 isolate 撞上 `gitstarclub-jobs-pre` 15 min Queue wall；未设 / `0` 仍是单 hop。输出 `data/whitelist.json`：`{id, node_id, full_name, owner, name, stars}`；其中 Search `stars` 是发现审计值，不是展示总数。默认 ≥10k 规模约 5.3k，每周变动。
