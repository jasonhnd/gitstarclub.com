...
- **Production Firewall denies training/scraper crawlers.** Four Vercel Firewall Deny rules (Meta external agent, GoogleOther, GPTBot training, SEO scrapers) are live on production only. `Googlebot`, `Bingbot`, `OAI-SearchBot`, and `ChatGPT-User` stay unblocked. The unclassified/tool rate-limit was not published (Security Plus required).
