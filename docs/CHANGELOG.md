...
- **Metadata GraphQL 502 no longer voids a refresh.** A transient GitHub `nodes()` failure (502/503/504/429 or fetch timeout) persists `ops/workflows/<run_id>/metadata-<bucket>.json` and re-enqueues the same bucket hop; the lease stays held. Completed batches are not refetched. After six hop-level transients the run can still fail closed. The next start can reuse an unpublished whitelist snapshot via `ops/workflows/latest-unpublished-whitelist.json` (including `refresh-2026-09-21T16-02-51-499Z` / 65015) instead of re-searching. Does not revert sharded Search, live-deploy, or trigger Bearer refresh. See [VERCEL-DATA-OPERATIONS.md](./VERCEL-DATA-OPERATIONS.md).
