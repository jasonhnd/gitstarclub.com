...
- **Sunday weekly live refresh no longer fences itself on a stale pointer read.** Control-plane claim / publish / release of `live/latest.json` now call `@vercel/blob` `get({ useCache: false })`. The weekly reuse path republishes in 1–2s; a cached pre-lease pointer made publish throw `live publication lease was fenced before commit` and left the lease stuck. Page readers still cache the pointer for 60s.
