...
- **Sunday weekly live refresh no longer fences itself on a stale pointer read.** Control-plane claim / publish / release fetch `live/latest.json?v=<now>` and write the pointer with `max-age=0`. `@vercel/blob` `useCache: false` is private-only, so it did not bypass the public CDN. The weekly reuse path republishes in 1–2s and otherwise re-read the pre-lease pointer. Page readers still memoize the generation for 60s.
