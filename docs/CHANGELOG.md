...
- **Hosting documentation (#528).** Production and preview web domains are served by Cloudflare Workers (OpenNext); Vercel Blob remains storage. The production refresh trigger is unresolved because the observed production Worker schedule was empty despite the owner reporting Cloudflare cron. The preview Worker had three schedules. See [OPS.md](./OPS.md) for dated evidence and the historical Vercel rollback reference.

- **Preview cold-start bootstrap for the first managed publish (#519).** `WORKFLOW_COLD_START=1` (wrangler `env.pre` only, with `PREFLIGHT_RELAX_EMPTY_SHARDS=1`) lets Bearer refresh enqueue when `canonical/v2/meta.json` and `views/latest.json` are absent: route preflight skips the legacy gate, workflow preflight writes minimal honest meta under lease, whitelist baseline is `[]`, and missing canonical / lookup shards read as empty until the first pointer publish. Node ids and stars still come only from GitHub Search + GraphQL; seam_date is the bootstrap UTC day, not a fabricated archive seam. After `views/latest.json` exists, cold-start read relaxation turns off even on pre. Production never arms. See [OPS.md](./OPS.md) and [VERCEL-DATA-OPERATIONS.md](./VERCEL-DATA-OPERATIONS.md). Closes #519.

- **Preview preflight no longer voids a refresh on missing/empty canonical shards.** `PREFLIGHT_RELAX_EMPTY_SHARDS=1` (wrangler `env.pre` only) treats confirmed 404 / empty buckets as empty `{}` placeholders under policy `preview-empty-canonical-placeholder`. Schema, identity, `d`, wrong-bucket, 403, and timeout still fail closed. `VERCEL_ENV=production` never relaxes. Does not revert sharded Search, live-deploy, or trigger Bearer refresh. See [OPS.md](./OPS.md) and [VERCEL-DATA-OPERATIONS.md](./VERCEL-DATA-OPERATIONS.md). Closes #515.

- **Metadata GraphQL 502 no longer voids a refresh.** A transient GitHub `nodes()` failure (502/503/504/429 or fetch timeout) persists `ops/workflows/<run_id>/metadata-<bucket>.json` and re-enqueues the same bucket hop; the lease stays held. Completed batches are not refetched. After twelve hop-level transients the run can still fail closed. The next start can reuse an unpublished whitelist snapshot via `ops/workflows/latest-unpublished-whitelist.json` (including `refresh-2026-09-21T16-02-51-499Z` / 65015) instead of re-searching; when that happens the new snapshot carries `metadata_resume_run_id` so GraphQL batch rows already written on the failed run are not refetched. Does not revert sharded Search, live-deploy, or trigger Bearer refresh. See [VERCEL-DATA-OPERATIONS.md](./VERCEL-DATA-OPERATIONS.md).

- **Metadata GraphQL 502 hop budget (#521).** Hop-level transient budget raised to 12 with 8s→60s backoff; cross-run resume via `metadata_resume_run_id` on reused whitelist snapshots. Closes #521.

- **Preview whitelist Search is multi-hop when `WHITELIST_SEARCH_SHARDS=1`.** ≥1k Search no longer runs in one `gitstarclub-jobs-pre` isolate (that path hit the 15 min Queue wall on `refresh-2026-09-21T14-05-03-479Z`). Each hop drains star-range buckets for ≤10 min (`WHITELIST_SEARCH_HOP_BUDGET_MS`), persists `ops/workflows/<run_id>/whitelist-search.json`, and only then writes the immutable snapshot. Unset / `0` restores the previous single-hop. Pages stay precomputed. Does not live-deploy, enable production CF Cron, or change `MIN_TRACKED_STARS`. See [OPS.md](./OPS.md) and [VERCEL-DATA-OPERATIONS.md](./VERCEL-DATA-OPERATIONS.md).
...
- **CF refresh lease renew no longer fails a still-owned fencing token after #499 week hops.** Preview died on `could not read a consistent origin ETag while renewing fencing token` once streamed week hops crossed isolate boundaries: #475 correctly withheld CAS when public GET and origin `head()` diverged, but the empty isolate write cache plus a CDN-stale GET exhausted five short retries. Lease read now prefers origin `getOrigin()` (private Blob hostname, one body+etag response). A successor on origin still fails closed. Week stream is unchanged. `WORKFLOW_RUNTIME=cf-queue` stays. Production `triggers.crons` stays `[]`. See [CF-MIGRATION-P1.md](./CF-MIGRATION-P1.md). Does not roll back cf-queue, stop Vercel cron, or cut DNS.

- **CF refresh week hop no longer assembles the full packed week window.** After #497, preview completed packed month/year hops and wrote `recompute/week-win/0..31`, then stuck on `recomputeRank-week-start`: fetch-origin `Worker exceeded memory limit` ×3, queue silent, no `recomputeRank-week` / publish. Week cells are ~4× month cells; loading every persist shard (or `finalize` of the full week builder) OOMs the 128 MiB isolate. Week is now a pack hop (one repo-bucket, flat v2 persist) plus streamed 8-period rank windows for week/weekOrg. `WORKFLOW_RUNTIME=cf-queue` stays. Production `triggers.crons` stays `[]`. See [CF-MIGRATION-P1.md](./CF-MIGRATION-P1.md). Does not roll back cf-queue, stop Vercel cron, or cut DNS.
