...
- **CF preview `/` and `/rankings` no longer 500 when live generation history exceeds 64 hops.** Period-scoped liveHistory reads still cap at 64 validated manifests and fail closed on cycle / schema / listed-but-missing. A request newer than the hop's `week`/`month` stops immediately (older hops cannot have that period). Hitting the hop bound without `previous_generation:null` now truncates to a miss so pages fall back to base / empty instead of throwing `live generation history exceeds 64 entries`. Does not raise the scan cap, change the refresh pipeline, enable production CF Cron, or stop Vercel cron. See [DATA-CONTRACTS.md](./DATA-CONTRACTS.md) §2.9. Closes #495.
