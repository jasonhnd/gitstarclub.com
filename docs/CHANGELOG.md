...
- **Cockpit glass HUD on `/cockpit` (pre only).** The posed radar spike now matches the cabin contract: search, This week / This month / This year, domain legend, always-visible three-column glass, and 90-day sparklines. The timeline is still full-history mouse scrub. The route stays `noindex` and off Chrome nav.
