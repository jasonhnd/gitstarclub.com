...
> v0.2 之后再加：主题聚类、相似推荐、用户系统、下钻到 ≥100 star 的"观察层"、任意 repo 对比（均 v0.3，DB 阻塞）。
...
- 🚧 多 repo 对比（**简版**，进行中）：≥1 万星集叠图对比（`/compare`，URL 即状态）；曲线复用已有 entity、瘦路由 `/repo-curve`、**零新数据产物 / 零 DB / 零外部依赖**

### v0.3 — 下钻与对比（数据库登场）

- 扩展数据集到 ≥100 star（46 万 repo）—— 单文件吃力，引入 **Tinybird (ClickHouse)**
- **任意 repo** 对比（含 ≥100 星长尾；简版 ≥1 万星对比已在 v0.2）
- 按语言 / topic / 创建年份的切片视图
