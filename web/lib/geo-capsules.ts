...
export function formatDataAsOf(value: string | null | undefined): string | null {
  if (!value) return null;
  const date = /^(\d{4})-(\d{2})-(\d{2})(?:T.*)?$/.exec(value);
  if (date) {
    return formatUtcDate(Number(date[1]), Number(date[2]), Number(date[3]));
  }
  const month = /^(\d{4})-(\d{2})$/.exec(value);
  if (month) {
    return monthYearLabel("en", Number(month[1]), Number(month[2]));
  }
...
export function buildRepoCapsule(repo: RepoEntity, asOf: string): AnswerCapsuleContent {
  ...
  const latestPhrase = latest ? `${monthYearLabel("en", Number(latest.month.slice(0, 4)), Number(latest.month.slice(5, 7)))} with ${signedStars(latest.adds)} stars` : "its latest precomputed monthly row";
  const text = withSource(
    `As of ${asOf}, ${repo.full_name} has ${fmtStars(repo.current_stars)} GitHub stars. GitStarClub tracks its ${language}profile, ${repoMilestonePhrase(repo)}, and latest recorded month of ${latestPhrase}, combining identity, milestone, current-star, and monthly curve fields for answerable repository history without runtime inference.`,
  );
  return capsule(text, asOf);
}

export function buildOrgCapsule(org: OrgEntity, asOf: string): AnswerCapsuleContent {
  const kind = org.owner_type === "Organization" ? "organization" : "developer";
  const text = withSource(
    `As of ${asOf}, ${org.login} has ${fmtStars(org.current_stars_sum)} total GitHub stars across ${org.repo_count.toLocaleString("en-US")} tracked repositories. GitStarClub builds this ${kind} page from precomputed organization JSON, member repository ids, current-star sums, and monthly curves so readers can cite organization momentum without a runtime database.`,
  );
  return capsule(text, asOf);
}
...
export function buildCategoryIndexCapsule(registry: CategoryRegistry, asOf: string): AnswerCapsuleContent {
  ...
  const text = withSource(
    `As of ${asOf}, GitStarClub organizes tracked GitHub repositories into ${publicCategories.length.toLocaleString("en-US")} public categories across ${registry.dimensions.length} dimensions, including ${labels}. These links come from deterministic category registry JSON and help readers reach focused repository lists without relying only on sitemap discovery.`,
  );
  return capsule(text, asOf);
}
...
export function buildCategoryDetailCapsule({
  ...
  const followers = second && third ? `, followed by ${repoName(second)} and ${repoName(third)}` : "";
  const text = withSource(
    `As of ${asOf}, GitStarClub tracks ${category.count.toLocaleString("en-US")} repositories in ${category.label}. ${leader}${followers}. This category ranking is generated from deterministic category assignment JSON, all-time stock ranking data, and repository lookup fields, not live search or AI.`,
  );
  return capsule(text, asOf);
}
...
function formatUtcDate(year: number, month: number, day: number): string {
  return new Intl.DateTimeFormat("en-US", { timeZone: "UTC", year: "numeric", month: "long", day: "numeric" }).format(Date.UTC(year, month - 1, day));
}

function repoMilestonePhrase(repo: RepoEntity): string {
  const milestones = [
    ["10k", repo.milestones.crossed_10k],
    ["50k", repo.milestones.crossed_50k],
    ["100k", repo.milestones.crossed_100k],
  ] as const;
  const known = milestones.flatMap(([label, date]) => (date ? [`${label} in ${monthYearLabel("en", Number(date.slice(0, 4)), Number(date.slice(5, 7)))}`] : []));
