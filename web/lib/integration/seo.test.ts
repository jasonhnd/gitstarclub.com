...
const liveDescribe = BASE === "" ? describe.skip.bind(describe) : describe.bind(describe);
const liveDescribeEach = BASE === "" ? describe.skip.each.bind(describe.skip) : describe.each.bind(describe);
...
] satisfies Array<{ label: string; path: string; canonPath: string }>;
...
liveDescribeEach(PAGES)("SEO basics — $label", ({ path, canonPath }) => {
...
liveDescribe("Open Graph image (og:image)", () => {
...
liveDescribe("document language", () => {
