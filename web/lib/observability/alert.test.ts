...
describe("sendAlert", () => {
  ...
  test("a webhook that rejects does NOT propagate", async () => {
    ...
    const logged = errSpy.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logged.some((l: string) => l.includes("webhook delivery failed"))).toBe(true);
  });
...
describe("recordHealth", () => {
  ...
  test("swallows a putView failure (does NOT propagate)", async () => {
    ...
    const logged = errSpy.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logged.some((l: string) => l.includes("health write failed"))).toBe(true);
  });
});
