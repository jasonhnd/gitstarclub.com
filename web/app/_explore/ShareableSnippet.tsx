...
export function ShareableSnippet({
  snippet,
  className = "",
  labels = { heading: "Shareable snippet", copy: "Copy", copied: "Copied", embed: "Embed", embedCopied: "Embed copied" },
}: {
  snippet: ShareableSnippetContent;
  className?: string;
  labels?: { heading: string; copy: string; copied: string; embed: string; embedCopied: string };
}) {
  return (
    <section aria-label={`${snippet.title} ${labels.heading}`} className={`rounded-2xl border border-outline-variant bg-surface-container px-4 py-4 ${className}`}>
