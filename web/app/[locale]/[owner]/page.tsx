...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; owner: string }> }): Promise<Metadata> {
  const { locale: owner, owner: name } = await params;
  return generateRepoMetadata({ locale: "en", owner, name });
}

export default async function RepoPage({ params }: { params: Promise<{ locale: string; owner: string }> }) {
  const { locale: owner, owner: name } = await params;
